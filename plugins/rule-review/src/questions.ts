...
function parseQuestion(value: unknown, file: string): QuestionDefinition {
  ...
  const subject = requiredString(value, "subject", file);
  if (subject !== "document" && subject !== "pair") {
    throw new Error(`${file}.subject must be document or pair`);
  }
  const at = probability(value, "at", file);
  const loose = probability(value, "loose", file);
  if (loose >= at) throw new Error(`${file}.loose must be lower than at`);
  return {
